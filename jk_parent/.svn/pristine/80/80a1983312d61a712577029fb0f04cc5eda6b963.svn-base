...
public class UserAction extends BaseAction implements ModelDriven<User>{
	...
	public String updateSelf() throws Exception {
		...
		//判断是否修改了密码,做不同的事情。true为密码修改了，false为密码没修改。功能还没写完，明天再改
		boolean flag = true;
		if(model.getPassword().equals(databaseUser.getPassword())){
			flag = false;
		}
		Set<Role> databaseroles = databaseUser.getRoles();
		Set<Board> databaseboards = databaseUser.getBoards();
		SetDataUtils.set(model, databaseUser);
		databaseUser.setRoles(databaseroles);
		databaseUser.setBoards(databaseboards);
